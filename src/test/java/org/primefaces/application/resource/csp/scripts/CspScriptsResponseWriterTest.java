...
public class CspScriptsResponseWriterTest {
    ...
    @Test
    public void testJavascriptUriHandlersWithoutIdAttribute() throws IOException {
        writer.startElement("body", null);

        for (String uriAttribute : URI_ATTRIBUTES) {
            writer.writeAttribute(uriAttribute, "javascript:alert('" + uriAttribute + "');", null);
        }
        verify(wrappedWriter, never()).writeAttribute(anyString(), anyString(), nullable(String.class));
        ...
        ArgumentCaptor<String> javascript = ArgumentCaptor.forClass(String.class);
        verify(wrappedWriter).writeText(javascript.capture(), nullable(String.class));
        Assert.assertEquals(URI_ATTRIBUTES.size(), countMatches(javascript.getValue(),"pf.csp2("));

        verify(wrappedWriter).endElement("script");
    }

    @Test
    public void testNonJavascriptUriHandlers() throws IOException {
        writer.startElement("body", null);

        for (String uriAttribute : URI_ATTRIBUTES) {
            writer.writeAttribute(uriAttribute, "http://primefaces.org/", null);
        }
        verify(wrappedWriter, times(URI_ATTRIBUTES.size())).writeAttribute(anyString(), anyString(), nullable(String.class));
