(function(window) {
    ...
    var PrimeFaces = {
        ...
        getFacesResource : function(name, library, version) {
           ...
           var scriptURI = PrimeFaces.getResourceScriptURI();
           var scriptName = PrimeFaces.getResourceScriptName(scriptURI);
        ...
        isExtensionMapping : function() {
           if (!PrimeFaces.IS_EXTENSION_MAPPING) {
              var scriptURI = PrimeFaces.getResourceScriptURI();
              var scriptName = PrimeFaces.getResourceScriptName(scriptURI);
              PrimeFaces.IS_EXTENSION_MAPPING = scriptURI.charAt(scriptURI.indexOf(scriptName) + scriptName.length) === '.';
           }
        ...
        getResourceUrlExtension : function() {
           if (!PrimeFaces.RESOURCE_URL_EXTENSION) {
              var scriptURI = PrimeFaces.getResourceScriptURI();
              var scriptName = PrimeFaces.getResourceScriptName(scriptURI);
              PrimeFaces.RESOURCE_URL_EXTENSION = RegExp(scriptName + '.([^?]*)').exec(scriptURI)[1];
           }

           return PrimeFaces.RESOURCE_URL_EXTENSION;
        },
        ...
        getResourceScriptName : function(scriptURI) {
           if (!PrimeFaces.SCRIPT_NAME) {
              ...
              PrimeFaces.SCRIPT_NAME = scriptRegex.exec(scriptURI)[2] + '.js';
           }

           return PrimeFaces.SCRIPT_NAME;
        },
        ...
        getResourceScriptURI : function() {
           if (!PrimeFaces.SCRIPT_URI) {
              PrimeFaces.SCRIPT_URI = $('script[src*="/' + PrimeFaces.RESOURCE_IDENTIFIER + '/"]').first().attr('src');
           }
           return PrimeFaces.SCRIPT_URI;
        },
