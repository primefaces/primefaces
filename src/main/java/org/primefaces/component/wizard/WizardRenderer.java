...
public class WizardRenderer extends CoreRenderer implements PartialRenderer {
	...
	protected void encodeNavigators(FacesContext facesContext, Wizard wizard) throws IOException {
		...
		writer.startElement("div", null);
		writer.writeAttribute("class", "ui-wizard-navbar", null);
		
		if(back == null) 
			encodeDefaultNavigator(facesContext, wizard, clientId + "_back", "Back", var + ".back()", "/primefaces/wizard/assets/prev.png", "ui-wizard-nav-back");
		else
			renderChild(facesContext, back);
		
		if(next == null) 
			encodeDefaultNavigator(facesContext, wizard, clientId + "_next", "Next", var + ".next()", "/primefaces/wizard/assets/next.png", "ui-wizard-nav-next");
		else
