    ...
    public void updateColumnsVisibility(FacesContext context) {
        ...
        if (columnTogglerParam != null) {
            ...
            for (String togglableColumnAsString : togglableColumnsArr) {
                ...
                UIColumn column = findColumn(togglableColumnAsString.substring(0, sepIndex));

                if (column != null) {
                    ((Column) column).setVisible(Boolean.valueOf(togglableColumnAsString.substring(sepIndex + 1)));
                }
            }
        }
    }
