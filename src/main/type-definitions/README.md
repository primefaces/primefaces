...
Broadly speaking, this is done as follows:

* Use [acorn](https://www.npmjs.com/package/acorn) to parse the JavaScript
  source code.
* Walk the sytnax tree and look for certain patterns, such as
  `PrimeFaces.widget.xxx = Class.extend({...})` and annotations such as
  `@interface` or `@function`. This produces a list of constant, objects, and
   functions that need to be documented.
* For each item to be documented, gather some information:
    ...
    * Create a typescript declaration from this data.
* Create one type declaration (`*.d.ts`) file with all documented items.
    * Run the generated type declaration file through
      [typescript](https://www.npmjs.com/package/typescript) to check for
      errors, such as types that do not exist (eg. a misspelled `Jquery`).
    ...
    * For each doc comment with an `@inheritdoc` tag, copy the description from
      the parent type(s) or method(s). This is done because there is no good
      suport for `@inheritdoc` by various tools.

## Build

Building is integrated into the maven build process, just run `mvn install`.
This will generate the type declaration file in the target directory
`target/type-decarlation/`. When the release profile `-P release` is specified,
the files will be output to `doc/<current_version>`.
...
may want to build directly via npm / node. To do so, first go to this directory,
then run:
...
# Generates the type declaration file and the typedocs
# Writes output to the given directory. For local development, use some path
# outside this project.
npm run generate-d-ts -- --outputDir /path/to/temporary/output/directory
```
...
* Widget `InputNumber`: AutoNumeric was updated to the current 4.x version.
* Widget `Printer`: The underlying printing library was exchanged. 
* Widget `Schedule`: The underlying AutoNumeric library was updated (should be done already). 
* Widget `Timeline`: The underlying library was updated (some method have changes: move, zoom, getSelectedId). 
...
* Can a link to the source in the type declaration file be included in the
  typdoc?
* Can header links be added to PF web page etc?
* Revise README.md and JSDOC.md
* Test that each widget is still working

(probably not)

* Add a type parameter to the `*WidgetCfg` for the type of the widget they
  belong to. This is necessary for the `behaviors` property that need to know
  about the widget type.
* Create a static `extend` property on the widget classes:
  `static extend: PrimeFaces.Class<AccordionPanel>["extend"];`

## Errors

Some errors I noticed while adding types:

* `ContextMenu` extends `Menu`. It has a method `ContextMenu#show(event)` that
  requires one parameter. But the method on base class `Menu#show` can be called
   without any paramters.
* `ColumnToggler` extends `DeferredWidget`, but does not override `_render`.
  Does it even need to extend DeferredWidget?
* The third-party lib `ContentFlow` for the widget of the same name pollutes the
  global environment by modifying the prototype of `HTMLElement` (eg.
  `addEvent`), `Event`, and more.
* The third party lib `datetimepicker` for the widget `Calendar` pollutes the
  global environment by modifying the prototype of `Date` (`getMicroseconds` /
  `setMicroseconds`).
* The `Paginator` widget should either call super during the init method (which
  appears not to be supported by the currently used `Class` implementation) or
  set the properties `id`, `jqId`, and `widgetVar` itself. Its `cfg.id` should
  be either a string (currently an array) or it should not extend `BaseWidget`.
