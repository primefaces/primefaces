...
PrimeFaces.widget.Calendar.prototype.configureOnSelectHandler = function() {
	...
	this.cfg.onSelect = function(dateText, input) {
		...
		if(!c.cfg.popup) {
			jQuery(c.jqId + '_input').val(dateText);
		}
		
		if(c.cfg.hasSelectListener) {
			var params = {};
			params[PrimeFaces.PARTIAL_PROCESS_PARAM] = c.id;
			
			if(c.cfg.onSelectUpdate) {
				params[PrimeFaces.PARTIAL_UPDATE_PARAM] = c.cfg.onSelectUpdate;
			}
			
			PrimeFaces.ajax.AjaxRequest(c.cfg.url, {
					formId:c.cfg.formId,
					global:true
				},
				params);
...
PrimeFaces.widget.CalendarExtensions = {
	...
	initialize : function() {
		...
		if(this.cfg.initialConfig.disabled) {
			this.disable();
		}
		
		if(this.isPopup() && !this.cfg.initialConfig.disabled) {
	...
	disable : function() {
		this.addMonthRenderer('1', this.renderOutOfBoundsDate);
		this.addMonthRenderer('2', this.renderOutOfBoundsDate);
		this.addMonthRenderer('3', this.renderOutOfBoundsDate);
		this.addMonthRenderer('4', this.renderOutOfBoundsDate);
		this.addMonthRenderer('5', this.renderOutOfBoundsDate);
		this.addMonthRenderer('6', this.renderOutOfBoundsDate);
		this.addMonthRenderer('7', this.renderOutOfBoundsDate);
		this.addMonthRenderer('8', this.renderOutOfBoundsDate);
		this.addMonthRenderer('9', this.renderOutOfBoundsDate);
		this.addMonthRenderer('10', this.renderOutOfBoundsDate);
		this.addMonthRenderer('11', this.renderOutOfBoundsDate);
		this.addMonthRenderer('12', this.renderOutOfBoundsDate);
	}
};
