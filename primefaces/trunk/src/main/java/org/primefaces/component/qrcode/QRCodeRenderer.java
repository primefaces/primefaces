 ...
 */
package org.primefaces.component.qrcode;
...
public class QRCodeRenderer extends CoreRenderer {
    ...
    private static final String SB_BUILD = QRCodeRenderer.class.getName() + "#build";
	...
	public void encodeEnd(FacesContext context, UIComponent component) throws IOException {
        ...
        String styleClass = qrCode.getStyleClass();
		String src = null;
        Object value = qrCode.getValue();
        ...
        try {
            ...
            String rid = encrypter.encrypt((String) qrCode.getValue());
            ...
            src = builder.append(resourcePath).append("&").append(Constants.DYNAMIC_CONTENT_PARAM).append("=").append(URLEncoder.encode(rid,"UTF-8"))
                    .append("&").append(Constants.DYNAMIC_CONTENT_TYPE_PARAM).append("=").append("qr").toString();
        } 
        catch (UnsupportedEncodingException ex) {
            throw new IOException(ex);
        }
        
		writer.startElement("img", qrCode);
        if(shouldWriteId(component)) writer.writeAttribute("id", clientId, "id");
        if(styleClass != null) writer.writeAttribute("class", styleClass, "styleClass");
        
        writer.writeAttribute("src", src, null);

        renderPassThruAttributes(context, qrCode, HTML.IMG_ATTRS);
