...
public class TerminalRenderer extends CoreRenderer implements PartialRenderer {
	...
	private void encodeScript(FacesContext facesContext, Terminal terminal) throws IOException {
		ResponseWriter writer = facesContext.getResponseWriter();
		String clientId = terminal.getClientId(facesContext);
	...
	public void encodePartially(FacesContext facesContext, UIComponent component) throws IOException {
		ResponseWriter writer = facesContext.getResponseWriter();
		Terminal terminal = (Terminal) component;
		String argsParam = facesContext.getExternalContext().getRequestParameterMap().get("args");
