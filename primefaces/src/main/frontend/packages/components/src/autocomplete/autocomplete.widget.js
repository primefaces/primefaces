...
PrimeFaces.widget.AutoComplete = class AutoComplete extends PrimeFaces.widget.BaseWidget {
    ...
    init(cfg) {
        ...
        this.cfg.active = this.cfg.active === undefined ? true : this.cfg.active;
        this.cfg.autoSelection = this.cfg.autoSelection === undefined ? true : this.cfg.autoSelection;
    ...
    bindDynamicEvents() {
        ...
        this.items.off('click.autocomplete mousedown.autocomplete mouseover.autocomplete')
            ...
            .on('click.autocomplete', function(event) {
                ...
                // #11570 allow CTRL+Click to select multiple
                if (!$this.cfg.multiple || !PrimeFaces.utils.isMetaKey(event)) {
                    $this.hide();
                }
            })
    ...
    showItemtip(item) {
        if (item.hasClass('ui-autocomplete-moretext')) {
            ...
            } else if (item.children('td:last').hasClass('ui-autocomplete-itemtip-content')) {
    ...
    showSuggestions(query) {
        ...
        if (this.items.length > 0) {
            ...
            if (this.cfg.showEmptyMessage && this.emptyMessage) {
    ...
    search(query) {
        ...
        else if (this.cfg.completeEndpoint) {
            $.ajax({
                ...
                if (suggestions.moreAvailable && $this.cfg.moreText) {
                    ...
                    html += '<li id="' + $this.id + '_item_more' + '" class="ui-autocomplete-item ui-autocomplete-moretext" role="option" data-item-label="'+$this.cfg.moreText+'">' + PrimeFaces.escapeHTML(moreTextEncoded, true) + '</li>';
                }
                html += '</ul>';
