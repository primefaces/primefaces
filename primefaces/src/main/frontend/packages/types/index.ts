declare global {
    namespace PrimeType {
        /**
         * The identity, just returns the type it is given. Useful e.g. for
         * the `extends` clause:
         * 
         * ```ts
         * const example = { foo(): void {} };
         * export interface Example extends Type<typeof example> {
         *   bar(): void;
         * }
         * ```
         *
         * @typeParam Type The type to return.
         * @returns The given type.
         */
        export type Type<Type> = Type;

        /**
         * Helper type for a newable, such as a class with a constructor.
         * @typeParam Args Type of the (constructor) arguments.
         * @typeParam Return Type of the return value, e.g. the constructed class.
         * @returns A newable that takes the given arguments and returns the given type.
         */
        export type Newable<Args extends unknown[], Return> = new (...args: Args) => Return;

        /**
         * Helper type for a newable to which any type of {@link Newable} can be assigned.
         */
         ...
         *
         * ```ts
         * type A = { bar: RegExp; foo: number; };
         * type B = RenameKeys<A, {bar: "b", foo: "f"}>;
         * // type B = { b: RegExp; f: number; }
         * ```
         ...
         */
        export type RenameKeys<Base, RenameMap> = {
            [Key in keyof Base as Key extends keyof RenameMap ? RenameMap[Key] extends string ? RenameMap[Key] : Key : Key]: Base[Key];
        };

        /**
         * Reverses the mapping from keys to strings.
         * 
         * ```ts
         * type A = { bar: "b"; foo: "f"; };
         * type B = InvertRecord<A>;
         * // type B = { b: "bar", f: "foo" }
         * ```
         */
    ...
    namespace faces {
         ...
         * number, reset with every increment of `faces.specversion` This number is implementation dependent.
         */
    ...
    namespace faces.ajax {
         ...
         * 
         * - `httpError`: Occurs when the status code is not 2xx or no status was returned.
         * - `serverError`: Occurs when the AJAX response contains an `error` element.
         * - `malformedXML`: Occurs when the AJAX response does not follow the proper format.
         * - `emptyResponse`: Occurs when there was no AJAX response from the server.
         */
         ...
         * - `complete`: Occurs immediately after the request has completed. For successful requests, this is
         * immediately before {@link response} is called. For unsuccessful requests, this is immediately before the
         * error handling callback is invoked.
         * - `success`: Occurs immediately after {@link response} has completed.
         */
        ...
        export interface CallbackData<T extends string, S extends string> {
            ...
            source: string;
            /**
             * HTTP response code of the request. Not present for `begin` event;
             */
            responseCode?: number;
            /**
             * XML of the response. Not present for `begin` event;
             */
            responseXML?: XMLDocument;
            /**
             * Raw text of the response. Not present for `begin` event;
             */
            responseText?: string;
        }
        /**
        ...
        /**
         * The request context that will be used during error/event handling. {@link response}.
         */
         ...
         * processing. Used by {@link request}.
         */
        export interface RequestOptions {
            /**
             * A space delimited list of client identifiers or one of the keywords. These reference the components that
             * will be processed during the `execute` portion of the request processing lifecycle.
             * 
             * Keywords are:
            ...
            /**
             * A space delimited list of client identifiers or one of the keywords. These reference the components that
             * will be processed during the `render` portion of the request processing lifecycle.
             *
             * Keywords are:
             * - `@all` All component identifiers.
             * - `@none` No identifiers.
        ...
        /**
         * Register a callback for error handling. The following events can be handled:
         * 
         * - `httpError`: Occurs when the status code is not 2xx or no status was returned.
         * - `serverError`: Occurs when the AJAX response contains an `error` element.
         * - `malformedXML`: Occurs when the AJAX response does not follow the proper format.
         ...
         * {@link addOnError} more than once. This function must throw an error if the callback argument is not a
         * function.
         * @param callback A reference to a function to call on an error.
         */
        export function addOnError(callback: OnErrorCallback): void;
        /**
         * The callback argument must be a reference to an existing JavaScript function that will handle the events. The
         * events that can be handled are:
         * 
         * - `begin`: Occurs immediately before the request is sent.
         * - `complete`: Occurs immediately after the request has completed. For successful requests, this is
         * immediately before {@link response} is called. For unsuccessful requests, this is immediately before the
         ...
         * This function must accept a reference to an existing JavaScript function. The JavaScript function reference must
         * be added to a list of callbacks, making it possible to register more than one callback by invoking
         * {@link addOnEvent} more than once. This function must throw an error if the callback argument is not a
         * function.
         ...
         *
         * - If the request completed successfully invoke {@link response} passing the request object.
