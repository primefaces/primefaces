...
| converter | null | Converter/String | An el expression or a literal text that defines a converter for the component. When itâ€™s an EL expression, itâ€™s resolved to a converter instance. In case itâ€™s a static text, it must refer to a converter id
...
## Localization
By default locale information is retrieved from the viewâ€™s locale and can be overridden by the locale
