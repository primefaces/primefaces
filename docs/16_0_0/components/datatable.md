...
| selectionPageOnly         | true               | Boolean          | When using a paginator and selection mode is `checkbox`, the select all checkbox in the header will select all rows on the current page if `true`, or all rows on all pages if `false`. When `false`, individual row unselections after "select all" are preserved even across page navigation. **Performance Warning**: When set to `false` with LazyDataModel and a selection binding, ALL data will be loaded into memory. Keep `true` (default) for large datasets to avoid this.
