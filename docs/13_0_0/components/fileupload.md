...
4. Consider **enabling virus scanning**. This feature has been introduced with PrimeFaces 7.0 and can be enabled with `p:fileUpload virusScan="true"`. See https://github.com/primefaces/primefaces/issues/4256.
   ...
   * **Built-in implementation**: ClamAV Daemon which can send a file over TCP to a running ClamAV service in your network. The default host/port for the ClamAV Daemon is localhost:3310. If you run the ClamAV Daemon on another host/port, you have to configure the host/port context params `primefaces.virusscan.CLAMAV_HOST` and `primefaces.virusscan.CLAMAV_PORT` in `web.xml` accordingly; More information at [ClamAV API](https://linux.die.net/man/8/clamd).
